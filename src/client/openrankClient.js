...
const getUsersWithSubscriptionsInChannel = async (channelName, limit = 0, orderUsersBy = 'total_points', stpContractAddress) => {
  ...
  // todo map users to users with subs
...
const getUsersInfoByAddresses = async (addresses) => {
  if (!addresses.length) {
    return []
  }

  // addresses = ['0xa6a8736f18f383f1cc2d938576933e5ea7df01a1']

  //https://api.neynar.com/v2/farcaster/user/bulk-by-address?addresses=0xa6a8736f18f383f1cc2d938576933e5ea7df01a1%2C0x7cac817861e5c3384753403fb6c0c556c204b1ce
  //https://api.neynar.com/v2/farcaster/user/bulk-by-address?addresses=0x41c7104Ca78815A0c2Be11BdE14a964547E3ce2b

  const url = `https://api.neynar.com/v2/farcaster/user/bulk-by-address?addresses=${encodeURIComponent(addresses.map(a => a.toLowerCase()).join(','))}`;
